...
<p class="text-gray-500">No cases found in <code>cases/*.json</code>.</p>`;
    ...
    const cards = cases.map((c) => {
        ...
        const variants = `<span class="px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-800">${c.variantCount} variant${c.variantCount === 1 ? "" : "s"}</span>`;
