...
function mapFhirToOmop(fhirResources: Condition[], mode?: "sequential" | "hash"): Record<string, unknown>[] {
  const ctx = new MappingContext(mode ? new IdRegistry(mode) : undefined);
  const results: Record<string, unknown>[] = [];

  for (const resource of fhirResources) {
    if (resource.resourceType === "Condition") {
      const result = mapCondition(resource, ctx);
      if (result) results.push({ table: "condition_occurrence", ...result });
    }
  }
...
for (const { name, cases } of testFiles) {
  describe(`condition/${name}.json (hash mode)`, () => {
    for (const tc of cases) {
      test(tc.description, () => {
