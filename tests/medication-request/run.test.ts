...
function mapFhirToOmop(fhirResources: MedicationRequest[], mode?: "sequential" | "hash"): Record<string, unknown>[] {
  const ctx = new MappingContext(mode ? new IdRegistry(mode) : undefined);
  const results: Record<string, unknown>[] = [];

  for (const resource of fhirResources) {
    if (resource.resourceType === "MedicationRequest") {
      const result = mapMedicationRequest(resource, ctx);
...
for (const { name, cases } of testFiles) {
  describe(`medication-request/${name}.json`, () => {
...
for (const { name, cases } of testFiles) {
  describe(`medication-request/${name}.json (hash mode)`, () => {
    for (const tc of cases) {
      test(tc.description, () => {
