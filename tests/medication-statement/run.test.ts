...
function mapFhirToOmop(fhirResources: MedicationStatement[], mode?: "sequential" | "hash"): Record<string, unknown>[] {
  const ctx = new MappingContext(mode ? new IdRegistry(mode) : undefined);
  const results: Record<string, unknown>[] = [];

  for (const resource of fhirResources) {
    if (resource.resourceType === "MedicationStatement") {
      const result = mapMedicationStatement(resource, ctx);
      if (result) results.push({ table: "drug_exposure", ...result });
    }
  }
...
for (const { name, cases } of testFiles) {
  describe(`medication-statement/${name}.json`, () => {
...
for (const { name, cases } of testFiles) {
  describe(`medication-statement/${name}.json (hash mode)`, () => {
    for (const tc of cases) {
      test(tc.description, () => {
