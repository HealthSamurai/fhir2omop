...
function mapFhirToOmop(fhirResources: AllergyIntolerance[], mode?: "sequential" | "hash"): Record<string, unknown>[] {
  const ctx = new MappingContext(mode ? new IdRegistry(mode) : undefined);
  const results: Record<string, unknown>[] = [];

  for (const resource of fhirResources) {
    if (resource.resourceType === "AllergyIntolerance") {
      const result = mapAllergyIntolerance(resource, ctx);
      if (result) results.push({ table: "observation", ...result });
    }
  }
...
for (const { name, cases } of testFiles) {
  describe(`allergy-intolerance/${name}.json`, () => {
...
for (const { name, cases } of testFiles) {
  describe(`allergy-intolerance/${name}.json (hash mode)`, () => {
    for (const tc of cases) {
      test(tc.description, () => {
