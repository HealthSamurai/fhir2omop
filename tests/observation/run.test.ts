...
import { mapObservation } from "../../src/mapper/observation";
...
function mapFhirToOmop(fhirResources: Observation[], mode?: "sequential" | "hash"): Record<string, unknown>[] {
  const ctx = new MappingContext(mode ? new IdRegistry(mode) : undefined);
  const results: Record<string, unknown>[] = [];

  for (const resource of fhirResources) {
    if (resource.resourceType === "Observation") {
      const { measurement, observation } = mapObservation(resource, ctx);

      if (measurement) {
        if (Array.isArray(measurement)) {
          for (const m of measurement) {
            results.push({ table: "measurement", ...m });
          }
        } else {
          results.push({ table: "measurement", ...measurement });
        }
      }
      ...
      if (observation) {
        if (Array.isArray(observation)) {
          for (const o of observation) {
            results.push({ table: "observation", ...o });
          }
        } else {
          results.push({ table: "observation", ...observation });
        }
      }
    }
  }
...
for (const { name, cases } of testFiles) {
  describe(`observation/${name}.json`, () => {
...
for (const { name, cases } of testFiles) {
  describe(`observation/${name}.json (hash mode)`, () => {
    for (const tc of cases) {
      test(tc.description, () => {
