...
The four event resources that DO produce `drug_exposure` rows are:
...
Vocabulary priority order for selecting the best coding: RxNorm > NDC > ATC > SNOMED > first available.
...
- `medicationReference` resolution is not implemented. Both MedicationRequest and MedicationStatement mappers return null when only `medicationReference` is present.
