...
4. **Date handling.** `authoredOn` maps to `drug_exposure_start_date`. `dispenseRequest.validityPeriod.end` maps to `drug_exposure_end_date` (fallback to start date when absent). FhirToCdm derives dates from the linked VisitOccurrence. NACHC falls back to encounter start date when `authoredOn` is null.
...
7. **Refills.** `MedicationRequest.dispenseRequest.numberOfRepeatsAllowed` maps to `refills`. omoponfhir reads this from the `dispenseRequest` block. Only applicable to MedicationRequest.
