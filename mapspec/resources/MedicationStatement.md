...
5. **Medication resolution.** Same strategies as MedicationRequest. `medicationCodeableConcept` vs `medicationReference` distinction. omoponfhir resolves contained references. ETL-German pre-indexes Medication resources via `medication_id_map`. This project handles inline only.
