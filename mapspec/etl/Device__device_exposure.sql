...
-- timing field. Synthea's FHIR export drops the START/STOP it writes
-- to devices.csv. We emit CURRENT_DATE as a placeholder so rows can be
-- written; downstream consumers should treat date=NULL semantics
-- accordingly. Real-world data should include DeviceUseStatement.

SELECT
    referenceToId(v.id)                                                     AS device_exposure_id,
    referenceToId(v.subject_ref)                                            AS person_id,
    std.concept_id                                                          AS device_concept_id,
    CURRENT_DATE                                                            AS device_exposure_start_date,
