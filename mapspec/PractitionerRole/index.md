...
|---|---|---|
| `PractitionerRole` | `provider` (enrichment) | Adds `specialty_concept_id`, `specialty_source_value`, and `care_site_id` to the Practitioner's existing provider row. |
...
- This is **enrichment, not row creation**. The base provider row must already exist (or be created on demand) from the Practitioner mapping. See [`../Practitioner/provider.md`](../Practitioner/provider.md).
...
- [provider](./provider.md) -- PractitionerRole-specific fields (specialty, care_site_id), specialty vocabulary, reference resolution, edge cases.
- [`../Practitioner/provider.md`](../Practitioner/provider.md) -- Base provider row created from Practitioner identity.
...
- **omoponfhir-v54** (Georgia Tech, Java) -- Does NOT process PractitionerRole as a separate resource; only the Practitioner resource is mapped. As a result, specialty is not populated by omoponfhir, and `care_site_id` is derived from `Practitioner.address` instead. See [`../Practitioner/provider.md`](../Practitioner/provider.md).
