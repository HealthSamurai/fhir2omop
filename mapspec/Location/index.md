...
- The `location` table is also populated from `Patient.address`; rows may need to be deduplicated by address components. See [`../Patient/location.md`](../Patient/location.md) for the Patient-driven path.
- For implementations that also derive care_site from Location: `Location.managingOrganization.reference` resolves the parent Organization's care_site_id; `Location.id` becomes the new care_site row's `location_id`. See [`./care_site.md`](./care_site.md).
...
- [location](./location.md) -- Primary mapping: Location → location table.
- [care_site](./care_site.md) -- Secondary pattern: Location → care_site (fhir-to-omop-demo dual-source approach).
- [`../Patient/location.md`](../Patient/location.md) -- Patient.address → location (shared table).
- [`../Organization/care_site.md`](../Organization/care_site.md) -- Primary care_site mapping from Organization.
