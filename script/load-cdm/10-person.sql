-- cdm.person + cdm.location — derived from patients.csv.
-- location_id == person_id == hash(Patient.id) so the diff aligns with
-- cdm_ours_fhir.* surrogate hashes.
...
SELECT
    ...
    "BIRTHDATE"::timestamp,
    CASE upper("RACE")
        WHEN 'WHITE' THEN 8527 WHEN 'BLACK' THEN 8516 WHEN 'ASIAN' THEN 8515
        ELSE 0
    END,
    CASE upper("ETHNICITY")
        WHEN 'HISPANIC' THEN 38003563 WHEN 'NONHISPANIC' THEN 38003564
        ELSE 0
    END,
    CASE WHEN "ZIP" IS NULL THEN NULL ELSE hashtextextended("Id", 0)::bigint END,
    NULL, NULL,
    -- source_concept_id columns mirror ETL-Synthea's hardcoded 0.
    "Id", "GENDER",    0,
    "RACE",            0,
    "ETHNICITY",       0
FROM _synthea_patients;
