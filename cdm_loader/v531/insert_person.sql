...
-- person_id = deterministic 64-bit hash of Patient UUID. Matches the
-- mint in mapspec/etl/Patient__person.sql so cdm.* and cdm_ours_fhir.*
-- agree on person_id per patient, enabling diff JOINs through it.
select
	hashtextextended(p.id, 0),
	case upper(p.gender)
		when 'M' then 8507
		when 'F' then 8532
	end,
