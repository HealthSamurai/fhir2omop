...
-- visit_occurrence_id = hash(encounter_id::text). For rolled-up IP/ER visits,
-- encounter_id is MIN(synthea encounter UUID) of the rolled group — that
-- specific UUID is what cdm_ours_fhir.* receives as Encounter.id for the
-- "primary" encounter of the group, so visit_occurrence_id matches there.
-- For collapsed children, FHIR side has a separate row keyed on a different
-- Encounter.id; those rows naturally diverge (expected rollup behavior).
SELECT *, hashtextextended(encounter_id::text, 0) as visit_occurrence_id
